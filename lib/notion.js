...
export async function getModelDetails(productSlug, modelName) {
  try {
    ...
    // Try with model_key first (new schema)
    ...
    // If no results with model_key, try with 'model' (old schema fallback)
    if (response.results.length === 0) {
      console.log('No results with model_key, trying with model field...');
    ...
    if (response.results.length > 0) {
      ...
      const modelDetails = {
        ...
        bannerImage: getPlainText(properties['bannerImage']) || '',
    ...
    console.log('No model found with either model_key or model field');
    return null;
  } catch (error) {
